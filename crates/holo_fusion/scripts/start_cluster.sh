...
# Secondary-index backfill knobs are intentionally separated from foreground DML
# so operators can push maintenance throughput without retuning OLTP traffic.
HOLO_FUSION_SCAN_PARALLELISM="${HOLO_FUSION_SCAN_PARALLELISM:-8}"
HOLO_FUSION_INDEX_BACKFILL_PAGE_SIZE="${HOLO_FUSION_INDEX_BACKFILL_PAGE_SIZE:-32768}"
HOLO_FUSION_INDEX_BACKFILL_WRITE_MAX_BATCH_ENTRIES="${HOLO_FUSION_INDEX_BACKFILL_WRITE_MAX_BATCH_ENTRIES:-4096}"
HOLO_FUSION_INDEX_BACKFILL_WRITE_MAX_BATCH_BYTES="${HOLO_FUSION_INDEX_BACKFILL_WRITE_MAX_BATCH_BYTES:-4194304}"
HOLO_FUSION_INDEX_BACKFILL_WRITE_PIPELINE_DEPTH="${HOLO_FUSION_INDEX_BACKFILL_WRITE_PIPELINE_DEPTH:-8}"
HOLO_FUSION_INDEX_BACKFILL_TARGET_PARALLELISM="${HOLO_FUSION_INDEX_BACKFILL_TARGET_PARALLELISM:-8}"
...
# Commit ACK durability and WAL fsync behavior for embedded holostore nodes.
HOLO_COMMIT_DURABILITY_MODE="${HOLO_COMMIT_DURABILITY_MODE:-sync_commit}"
HOLO_WAL_PERSIST_MODE="${HOLO_WAL_PERSIST_MODE:-sync_data}"
HOLO_WAL_PERSIST_ASYNC="${HOLO_WAL_PERSIST_ASYNC:-false}"
...
start_node() {
  ...
  local -a cmd=(
    ...
    HOLO_FUSION_SCAN_PARALLELISM="$HOLO_FUSION_SCAN_PARALLELISM"
    HOLO_FUSION_INDEX_BACKFILL_PAGE_SIZE="$HOLO_FUSION_INDEX_BACKFILL_PAGE_SIZE"
    HOLO_FUSION_INDEX_BACKFILL_WRITE_MAX_BATCH_ENTRIES="$HOLO_FUSION_INDEX_BACKFILL_WRITE_MAX_BATCH_ENTRIES"
    HOLO_FUSION_INDEX_BACKFILL_WRITE_MAX_BATCH_BYTES="$HOLO_FUSION_INDEX_BACKFILL_WRITE_MAX_BATCH_BYTES"
    HOLO_FUSION_INDEX_BACKFILL_WRITE_PIPELINE_DEPTH="$HOLO_FUSION_INDEX_BACKFILL_WRITE_PIPELINE_DEPTH"
    HOLO_FUSION_INDEX_BACKFILL_TARGET_PARALLELISM="$HOLO_FUSION_INDEX_BACKFILL_TARGET_PARALLELISM"
    ...
    HOLO_COMMIT_DURABILITY_MODE="$HOLO_COMMIT_DURABILITY_MODE"
    HOLO_WAL_PERSIST_MODE="$HOLO_WAL_PERSIST_MODE"
    HOLO_WAL_PERSIST_ASYNC="$HOLO_WAL_PERSIST_ASYNC"
  )
