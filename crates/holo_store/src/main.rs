...
///
/// Purpose:
/// - Keep client reads and writes behind range-operation fences until routing
///   metadata and shard ownership are safe to use.
///
/// Design:
/// - `Write` blocks on every fence reason.
/// - `Read` also blocks on every fence reason; split fences are correctness
///   barriers because routing during cutover can otherwise observe stale shard
///   state.
///
/// Inputs:
...
impl KeyFenceWaitMode {
    ...
    ///
    /// Purpose:
    /// - Preserve linearizable reads and writes while routing is being changed
    ///   by split and rebalance workflows.
    ///
    /// Design:
    /// - Reads and writes keep strict blocking semantics for all fences.
    ///
    /// Inputs:
    ...
    fn blocks_on_fence(self, _reason: &str) -> bool {
        match self {
            Self::Write => true,
            Self::Read => true,
        }
...
impl NodeState {
    ...
    ///
    /// Purpose:
    /// - Prevent reads and writes from racing split/merge control flow.
    ///
    /// Design:
    /// - Poll cluster metadata and evaluate only shards touched by this request.
    /// - Both reads and writes block on every active fence. Split fences protect
    ///   routing cutover, so bypassing them can produce stale reads.
    ///
    /// Inputs:
        ...
        loop {
            ...
            let blocked = keys.iter().find_map(|key| {
                ...
                snapshot
                    .shard_fences
                    .get(&shard_id)
                    // All active fences block client requests. Split fences are
                    // short-lived routing barriers, not optional slow paths.
    ...
    /// Execute a client batch SET directly.
    ...
    ///
    /// Inputs:
    /// - `handle`: target Accord group handle.
    /// - `kind`: proposal kind used for metrics.
    /// - `command`: encoded state-machine command.
    ///
    /// Outputs:
    /// - Accord proposal result with timing counters updated.
...
mod tests {
    ...
    /// Validate read-mode fence policy for split and non-split fences.
    ///
    /// Purpose:
    /// - Ensure GET requests block on split workflow fences and other control
    ///   fences before routing.
    ///
    /// Design:
    /// - Assert both split and non-split classifications block for `Read`.
    ///
    /// Inputs:
    ...
    #[test]
    fn key_fence_wait_mode_read_blocks_all_fences() {
